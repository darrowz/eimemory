...
| Timer | Purpose |
| --- | --- |
| `eimemory-code-implementation-refresh.timer` | Refresh the exact Hermes `code.implementation:v8` live-health advertisement every 20 minutes (TTL 3600 seconds). It does not run incubation or own lifecycle acceptance state. |
