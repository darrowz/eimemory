...
_restart_storage_writers() {
  ...
  local unit core current_release core_failed=0
  # During recovery, each core gets an independent start attempt. In particular,
  # a failed RPC must not strand the Hermes messaging gateway offline.
  for core in eimemory-rpc.service openclaw-gateway.service hermes-gateway.service; do
    for unit in "${ACTIVE_STORAGE_WRITER_UNITS[@]}"; do
      ...
      case "$unit" in
        eimemory-rpc.service)
          current_release="$(realpath -e -- "$CURRENT_LINK")" || { core_failed=1; continue; }
          ...
          ;;
        openclaw-gateway.service)
          if ! _wait_openclaw_gateway_ready; then core_failed=1; fi
          ;;
      esac
    done
  done
