...
const DEFAULT_OPERATOR_USER_ID = process.env.EIMEMORY_USER_ID || process.env.USER || 'operator';
...
const DEFAULT_REPLY_DELIVERY_STATE_PATH = process.env.EIMEMORY_REPLY_DELIVERY_STATE_PATH || require('node:path').join(process.env.EIMEMORY_ROOT || require('node:path').join(require('node:os').homedir(), '.openclaw', 'memory', 'eimemory'), 'openclaw_reply_delivery_state.json');
const DEFAULT_REPLY_DELIVERY_ATTEMPTS_PATH = process.env.EIMEMORY_REPLY_DELIVERY_ATTEMPTS_PATH || require('node:path').join(process.env.EIMEMORY_ROOT || require('node:path').join(require('node:os').homedir(), '.openclaw', 'memory', 'eimemory'), 'openclaw_reply_delivery_attempts.json');
const DEFAULT_RELEASE_CLOSURE_SIGNAL_PATH = process.env.EIMEMORY_RELEASE_CLOSURE_SIGNAL_PATH || require('node:path').join(process.env.EIMEMORY_ROOT || require('node:path').join(require('node:os').homedir(), '.openclaw', 'memory', 'eimemory'), 'state', 'release-closure-channel-receipt.signal');
...
function updateReplyDeliveryState(update) {
  try {
    ...
    const afterCommit = [];
    update(state, afterCommit);
    if (writeReplyDeliveryState(state)) {
      for (const commitAction of afterCommit) {
        try {
          commitAction();
        } catch (error) {
          console.warn(`eimemory reply delivery commit cleanup failed: ${String(error?.message || error)}`);
        }
      }
    }
  } catch (error) {
