...
def retrieval_stage_diagnostics(explanation):
    ...
    def stage(value):
        if not isinstance(value, dict):
            return {}
        result = {}
        for key in ('status', 'reason', 'error_type', 'error_reason', 'fallback_reason'):
            if value.get(key):
                result[key] = label(value[key])
        for key in ('candidate_count', 'candidate_limit', 'selected_count', 'elapsed_ms', 'calls'):
            if key in value:
                result[key] = number(value[key])
        for key in ('drops', 'dropped_reasons'):
