...
Schema/digest/feature helpers live in ``real_query_schema`` (A2). Baseline and
engine import facades are ``real_query_baseline`` / ``real_query_engine``.
This module retains orchestration, release-binding, and sanitized evidence
implementation behind the production_recall evaluator entry point.
"""
...
from eimemory.evaluation.real_query_schema import (  # A2: shared schema/digest layer
    ...
    PRODUCTION_REAL_QUERY_REQUIRED_CHANNELS,
    PRODUCTION_REAL_QUERY_DATASET_EVIDENCE_SCHEMA,
    PRODUCTION_RECALL_BOOTSTRAP_STATE_SCHEMA,
    PRIOR_HEALTH_SNAPSHOT_SCHEMA,
    ...
    _REAL_QUERY_MIN_ACTIVE_CHANNELS,
    _REAL_QUERY_REQUIRED_PER_CHANNEL,
    _REAL_QUERY_MIN_CASES,
    _REAL_QUERY_MIN_LABELS,
    _REAL_QUERY_RECALL_DEPTH,
    _MAX_QUERY_TERMS,
    _MAX_QUERY_TERM_CHARS,
    _MAX_QUERY_FEATURE_CHARS,
    _MAX_BASELINE_CHAIN_DEPTH,
    _MAX_PRIOR_HEALTH_SNAPSHOT_BYTES,
    _RECALL_CORPUS_KINDS,
    _LOW_SIGNAL_QUERY_TOKENS,
    _DIGEST_KEYS,
    _RAW_FIELD_MARKERS,
    production_real_query_active_channel_contract,
    production_real_query_policy_payload,
    production_real_query_policy_digest,
    _sample_channel_counts,
    _bounded_window,
    _parse_bounded_timestamp,
    _secure_dataset_evidence,
    _bounded_query_features,
    production_real_query_feature_quality_reasons,
    _query_feature_tokens,
    _looks_like_secret,
    _looks_like_high_confidence_phone,
    _normalize_decimal_digits,
    _looks_like_date_time_candidate,
    _contains_person_entity,
    _stable_digest,
    _engine_identity_digest,
)
...
def _trusted_result_capacity(runtime: Any, *, scope: ScopeRef, source_id: str) -> int | None:
    ...
    try:
        ...
        _LOG.warning("real_query_capacity_counter_failed: %s", exc)
        return None
...
def _retrieval_identity(runtime: Any, *, samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if callable(identity_provider):
        try:
            ...
            _LOG.warning("real_query_engine_identity_failed: %s", exc)
            raw_identity = None
...
def _production_proactive_metrics(runtime: Any, cases: list[dict[str, Any]]) -> dict[str, Any]:
    totals: dict[str, Any] = {key: 0 for key in ("volunteered", "injected", "used", "not_used", "rejected", "control")}
    totals["blocked_reasons"] = []
    ...
    for case in cases:
        ...
        try:
            ...
            _LOG.warning(
                "real_query_proactive_metrics_failed channel=%s source=%s: %s",
                case.get("channel"),
                case.get("source_id"),
                exc,
            )
            totals.setdefault("blocked_reasons", [])
            # store as parallel list-ish via join key count
            totals["blocked_reasons"] = list(
                dict.fromkeys(
                    [
                        *list(totals.get("blocked_reasons") or []),
                        f"proactive_metrics_unavailable:{case.get('channel')}:{case.get('source_id')}",
                    ]
                )
            )
            continue
