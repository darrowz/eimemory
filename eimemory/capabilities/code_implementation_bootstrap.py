"""Explicit registration of the immutable Hermes code-implementation v11 facts."""
...
CODE_IMPLEMENTATION_BOOTSTRAP_SCHEMA = "code.implementation.bootstrap.v11"
...
def code_implementation_revision() -> CapabilityRevision:
    return CapabilityRevision(
        ...
        evidence_refs=("bootstrap://code-implementation-v11-contract",),
    )
...
def code_implementation_binding(*, implementation_digest_value: str = "") -> CapabilityBinding:
    ...
    return CapabilityBinding(
        ...
        advertisement_evidence_refs=("bootstrap://code-implementation-v11-binding",),
        provenance={
            "source": "eimemory.code_implementation_bootstrap",
            "schema": CODE_IMPLEMENTATION_BOOTSTRAP_SCHEMA,
            "manual_bootstrap": True,
...
def register_code_implementation_v2(
    ...
    *,
    ...
    """Register the v11 immutable facts and preserve superseded revisions."""
    ...
    try:
        revision_receipt = runtime.capabilities.register_revision(
            revision,
            runtime_scope=scope,
            request_key=f"code-implementation-v11:revision:{revision.contract_digest}",
        )
        binding_receipt = runtime.capabilities.bind(
            binding,
            runtime_scope=scope,
            request_key=f"code-implementation-v11:binding:{binding.binding_digest}",
        )
    ...
    # Register v11 first, then preserve prior facts through lifecycle events.
    try:
    ...
    for superseded_revision_id in SUPERSEDED_REVISION_IDS:
        ...
        try:
            transition = runtime.capabilities.transition_status(
                ...
                request_key=(
                    f"code-implementation-v11:deprecate:{superseded_revision_id}:"
    ...
    result = service.advertise_capabilities(
        {
            ...
            "evidence_refs": [
                "bootstrap://code-implementation-v11-advertisement",
