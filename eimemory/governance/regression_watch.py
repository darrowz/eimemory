...
def run_regression_watch(
    ...
    *,
    ...
    if not regressed:
        return {"ok": True, "regressed": False, "record_id": ""}
    ...
    record = append_learning_record_once(
        ...
        summary=f"Regression detected for {candidate.record_id}",
        ...
        authority_tier=tier,
        status="active",
        content={"candidate_id": candidate.record_id, "eval_result": eval_result, "action": action, "rollback": candidate.content.get("rollback") or "disable candidate"},
        meta={"candidate_id": candidate.record_id, "action": action, "authority_tier": tier},
    )
    return {"ok": True, "regressed": True, "action": action, "record_id": record.record_id}
