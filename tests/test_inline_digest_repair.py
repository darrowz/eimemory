...
def test_timestamp_repair_preserves_verified_envelope(tmp_path,legacy):
    ...
    try:
        store.append(record)
        if legacy:
            store.sqlite.conn.execute('UPDATE records SET payload_digest=? WHERE record_id=?',('',record.record_id))
        ...
        fixed=repair_inline_projection_timestamps(store,scope=scope,apply=True)
        assert fixed['repaired']==1
        assert fixed['changes'][0]['payload_integrity']==('legacy_no_checksum' if legacy else 'checksum_verified')
