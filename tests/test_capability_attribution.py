...
def test_legacy_office_evidence_remains_diagnostic_only(tmp_path) -> None:
    runtime = Runtime.create(root=tmp_path)
    ...
    assert item["goal_gap_reason"] == "no_outcome_evidence"
...
def test_attribution_covers_research_synthesis_and_device_control(tmp_path) -> None:
    ...
    assert ledger["capabilities"]["device.control"]["evidence_count"] == 3


def test_attribution_keeps_chinese_keyword_search_as_diagnostics(tmp_path) -> None:
    runtime = Runtime.create(root=tmp_path)
...
def test_legacy_diagnostics_are_not_written_to_capability_ledger(tmp_path) -> None:
    runtime = Runtime.create(root=tmp_path)
    scope = {"agent_id": "hongtu", "workspace_id": "legacy-ledger"}
    try:
        for index in range(3):
            record_outcome_trace(
                runtime,
                _trace_payload(
                    f"legacy-ledger-{index}",
                    task_type="search_discovery",
                    summary="Recent GitHub source discovery was verified",
                ),
                scope=scope,
    ...
    assert len(diagnostics["search.discovery"]) == 3
    assert all(item["contract_verified"] is False for item in diagnostics["search.discovery"])
    assert "search.discovery" not in report["capabilities"]
    assert ledger["capabilities"]["search.discovery"]["evidence_count"] == 0
